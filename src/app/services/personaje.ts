...
export class PersonajeService {
  ...
  private personajesMarvel: Personaje[] = [
  {
    nombre: "Tony Stark",
    alias: "Iron Man",
  ...
  getPersonajes(): Observable<Personaje[]> {
    ...
    return of(this.personajesMarvel);
  }


  getPersonajesPorRango(inicio: number, fin: number): Observable<Personaje[]> {
    const listaRecortada = this.personajesMarvel.slice(inicio, fin);
    return of(listaRecortada);
  }


  getPersonaje(indice: number): Observable<Personaje | undefined> {
    const personajeEncontrado = this.personajesMarvel[indice];
    return of(personajeEncontrado);
  }
}